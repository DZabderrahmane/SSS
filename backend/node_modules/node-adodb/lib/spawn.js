 ...
 * @param {Object} options Spawn configuration
...
const spawn = function(command, args, input, options) {
  return new Promise((resolve, reject) => {
    ...
    // Delete options.encoding so it is not passed further
    delete options.encoding;

    // *** Modification: Force use of 64-bit cscript.exe if the command is cscript.exe ***
    if (typeof command === 'string' && command.toLowerCase() === 'cscript.exe') {
    ...
    child.on('close', exitCode => {
      // Set exit message if exitCode is not 0
      if (exitCode !== 0) {
        errors.exitMessage = exitMessages[exitCode];
      }

      // If there is stderr output, capture it
      ...
      // Otherwise, resolve with the output data
